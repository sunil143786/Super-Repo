...
class Database:
    ...
    async def generate_code(self, duration_in_seconds):
        code = ''.join(random.choices(string.ascii_uppercase + string.digits, k=10))
        hashed_code = self.hash_code(code)
        expiry_time = datetime.now() + timedelta(seconds=duration_in_seconds)
        code_data = {
    ...
    async def redeem_code(self, user_id, code):
        hashed_code = self.hash_code(code)
        code_data = await self.codes.find_one({"code_hash": hashed_code})
 ...
 #files Counting system, thanks🙏 for @safaridev
    ...
    #group verification system Or leave system
    ...
    # group verification Or leave system end
