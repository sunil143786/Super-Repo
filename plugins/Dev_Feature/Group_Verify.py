...
@Client.on_message(filters.command("verify_grp") & filters.private & filters.user(ADMINS))
async def verify_grp_command(client, message):
    if len(message.command) != 2:
        await message.reply("Usage: /verify_grp <group_id>")
        return
    ...
    if not group_info:
        await message.reply("ɢʀᴏᴜᴘ ɴᴏᴛ ғᴏᴜɴᴅ!")
        return
...
# Callback query handler for group unverification
@Client.on_callback_query(filters.regex(r"^un_rejected_"))
async def unverify_group_callback(client, query):
    ...
    await query.message.edit_text(f"<b>𝑩𝒐𝒕: {temp.U_NAME}\n𝑮𝒓𝒐𝒖𝒑: <a href={group_link}>{group_title}</a>\n𝑰𝑫: {chat_id}\n𝑴𝒆𝒎𝒃𝒆𝒓𝒔: {total}</b>\n\nGʀᴏᴜᴘ Is Vᴇʀɪғɪᴇᴅ. ✅</b>", reply_markup=InlineKeyboardMarkup(
        [[InlineKeyboardButton("Rᴇᴊᴇᴄᴛ ⛔", callback_data=f"rejected_group_{chat_id}")]]
    ))
    if owner_id:
        await client.send_message(chat_id=owner_id, text=f"#𝐕𝐞𝐫𝐢𝐟𝐲𝐞𝐝_𝐆𝐫𝐨𝐮𝐩\n\nGʀᴏᴜᴘ Nᴀᴍᴇ: {group_title}\nIᴅ: {chat_id}\n\nCᴏɴɢʀᴀᴛᴜʟᴀᴛɪᴏɴs Gʀᴏᴜᴘ Is Vᴇʀɪғɪᴇᴅ. ✅")

# Callback query handler for group verification
...
async def rejected_group_callback(client, query):
    ...
    await query.message.edit_text(f"𝑩𝒐𝒕: {temp.U_NAME}\n𝑮𝒓𝒐𝒖𝒑: <a href={group_link}>{group_title}</a>\n𝑰𝑫: {chat_id}\n𝑴𝒆𝒎𝒃𝒆𝒓𝒔: {total}</b>\n\nRᴇᴊᴇᴄᴛᴇᴅ Gʀᴏᴜᴘ ❌", reply_markup=InlineKeyboardMarkup(
        [[InlineKeyboardButton("Tᴀᴘ Tᴏ Vᴇʀɪғʏ ✅", callback_data=f"un_rejected_{chat_id}")]]
    ))
    if owner_id:
