...
from info import *
from utils import get_settings, get_size, is_subscribed, is_req_subscribed, save_group_settings, temp,  get_tutorial, get_shortlink, get_seconds
    ...
    try: 
        ...
        if len(message.command) != 2:
            buttons = [[
                        ...
                        InlineKeyboardButton('⚙️ᴜᴘᴅᴀᴛᴇꜱ ⚙️', callback_data='channels'), 
        ...
        if len(message.command) == 2 and message.command[1] in ["subscribe", "error", "okay", "help"]:
            buttons = [[
                        ...
                        InlineKeyboardButton('⚙️ᴜᴘᴅᴀᴛᴇꜱ ⚙', callback_data='channels'), 
                        InlineKeyboardButton('⚔️ғᴇᴀᴛᴜʀᴇs ⚔️', callback_data='features')
                    ],[
                        InlineKeyboardButton('☠️ʙᴏᴛ ᴄᴍᴅ ☠️', callback_data='help'),
                        InlineKeyboardButton('🤖 ᴀʙᴏᴜᴛ 🤖', callback_data='about')
                    ],[
                        InlineKeyboardButton('📣 ʀᴇғғᴇʀ: ɢᴇᴛ ᴘʀᴇᴍɪᴜᴍ 👑', callback_data="pm_reff")
                    ],[
        ...
        if not await db.has_premium_access(user_id):
            ...
            if (not user_verified or is_second_shortener or is_third_shortener) and settings.get("is_verify", IS_VERIFY):
                ...
                tutorial = settings.get('tutorial3', TUTORIAL3) if is_third_shortener else settings.get('tutorial2', TUTORIAL2) if is_second_shortener else settings['tutorial']
                ...
                buttons = [[
                    ...
                    InlineKeyboardButton("ɴᴏ ɴᴇᴇᴅ ᴛᴏ ᴠᴇʀɪғʏ 😁", callback_data='premium_info')
                ]]
                reply_markup=InlineKeyboardMarkup(buttons) 
                if await db.user_verified(user_id): 
        ...
        if data and data.startswith("allfiles"):
            ...
            for file in files:
                ...
                if not await db.has_premium_access(message.from_user.id):
                    ...
                    if settings.get("filelock", FILES_LIMITE_MODE):
                        ...
                        if send_count is not None and send_count >= limit:
                            ...
                            reply_markup = InlineKeyboardMarkup(buttons)
                            return await message.reply_text(script.BUTTON_LIMIT.format(hours, minutes, seconds),
                            reply_markup=reply_markup)
                button = [[
                ...
                if not await db.has_premium_access(message.from_user.id):
                    ...
                    if settings.get("filelock", FILES_LIMITE_MODE):
                        ...
                        if files_counts is not None and files_counts >= limit:
                            ...
                            reply_markup = InlineKeyboardMarkup(buttons)
                            return await message.reply_text(script.FILE_LIMIT.format(hours, minutes, seconds),
                            reply_markup=reply_markup)
                button = [[
                    InlineKeyboardButton("🖥️ ᴡᴀᴛᴄʜ / ᴅᴏᴡɴʟᴏᴀᴅ 📥", callback_data=f"streaming#{file_id}#{grp_id}")
                    ]]
...
async def settings(client, message):
    ...
    if settings is not None:
        buttons = [
            ...
            [
                InlineKeyboardButton('Fɪʟᴇ Lɪᴍɪᴛ', callback_data=f'setgs#filelock#{settings.get("filelock", FILES_LIMITE_MODE)}#{grp_id}'),
                InlineKeyboardButton('𝕋𝕣𝕦𝕖 ✔️' if settings.get("filelock", FILES_LIMITE_MODE) else '𝔽𝕒𝕝𝕤𝕖 ✗', callback_data=f'setgs#filelock#{settings.get("filelock", FILES_LIMITE_MODE)}#{grp_id}'),
            ], 
            [
                InlineKeyboardButton('Sᴛʀᴇᴀᴍ Sʜᴏʀᴛ', callback_data=f'setgs#stream_mode#{settings.get("stream_mode", STREAM_MODE)}#{grp_id}'),
                InlineKeyboardButton('𝕋𝕣𝕦𝕖 ✔️' if settings.get("stream_mode", FILES_LIMITE_MODE) else '𝔽𝕒𝕝𝕤𝕖 ✗', callback_data=f'setgs#stream_mode#{settings.get("stream_mode", STREAM_MODE)}#{grp_id}'),
            ], 
            [
                InlineKeyboardButton('Vᴇʀɪғʏ', callback_data=f'setgs#is_verify#{settings.get("is_verify", IS_VERIFY)}#{grp_id}'),
                InlineKeyboardButton('𝕋𝕣𝕦𝕖 ✔️' if settings.get("is_verify", IS_VERIFY) else '𝔽𝕒𝕝𝕤𝕖 ✗', callback_data=f'setgs#is_verify#{settings.get("is_verify", IS_VERIFY)}#{grp_id}'),
            ],
        ]
        btn = [[
