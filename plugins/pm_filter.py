...
from utils import get_size, is_subscribed, get_poster, temp, get_settings, save_group_settings, get_shortlink, stream_site, get_tutorial, get_text, imdb
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data == "close_data":
            ...
            buttons = [
                ...
                [
                    InlineKeyboardButton('Fɪʟᴇ Lɪᴍɪᴛ', 
                                        callback_data=f'setgs#filelock#{settings.get("filelock", FILES_LIMITE_MODE)}#{grp_id}'),
                    InlineKeyboardButton('ᴏɴ ✔️' if settings.get("filelock", FILES_LIMITE_MODE) else 'ᴏғғ ✗', 
                                        callback_data=f'setgs#filelock#{settings.get("filelock", FILES_LIMITE_MODE)}#{grp_id}')
                ],
                [  
                     InlineKeyboardButton('Sᴛʀᴇᴀᴍ Sʜᴏʀᴛ', 
                                        callback_data=f'setgs#stream_mode#{settings.get("stream_mode", STREAM_MODE)}#{grp_id}'),
                    InlineKeyboardButton('𝕋𝕣𝕦𝕖 ✔️' if settings.get("stream_mode", FILES_LIMITE_MODE) else '𝔽𝕒𝕝𝕤𝕖 ✗', 
                                        callback_data=f'setgs#stream_mode#{settings.get("stream_mode", STREAM_MODE)}#{grp_id}')
                ], 
                [
                    InlineKeyboardButton('Vᴇʀɪғʏ', 
                                        callback_data=f'setgs#is_verify#{settings.get("is_verify", IS_VERIFY)}#{grp_id}'), 
                    InlineKeyboardButton('𝕋𝕣𝕦𝕖 ✔️' if settings.get("is_verify", IS_VERIFY) else '𝔽𝕒𝕝𝕤𝕖 ✗', 
                                        callback_data=f'setgs#is_verify#{settings.get("is_verify", IS_VERIFY)}#{grp_id}')
                ]
            ]
            reply_markup = InlineKeyboardMarkup(buttons)
        ...
        buttons = [[
            InlineKeyboardButton('• QR •', callback_data='qr_info'),
        ...
        if await db.has_premium_access(query.from_user.id):
            await query.answer("ʏᴏᴜ ᴀʟʀᴇᴀᴅʏ ʜᴀᴠᴇ ᴀ ᴘʟᴀɴ 🙂", show_alert=True)      
            return
        elif has_free_trial:
            ...
            await query.message.reply_text(
                ...
                disable_web_page_preview=True,                  
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("💸 ᴄʜᴇᴄᴋᴏᴜᴛ ᴘʟᴀɴꜱ 💸", callback_data='premium_info')]]))
            await query.message.delete()
            return    
